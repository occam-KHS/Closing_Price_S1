...
def select_stocks(today_dt):
    ...
    X = model_inputs.loc[today_dt][['code','name','return','kosdaq_return','close','high'] + feature_list].set_index('code')
    ...
    tops = X[X['yhat'] >= 0.35].sort_values(by='yhat', ascending=False) # 스코어 0.3 이상 종목만
    print(len(tops))    
    ...
    select_tops = tops[(tops['return']>1) & (tops['high']/tops['close'] < 1.015) & (tops['volume_z'] > 1.1)]
